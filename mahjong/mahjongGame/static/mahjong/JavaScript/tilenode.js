...
class TileNode {
     ...
     * @see Layout
     */
    constructor(x, y, height, numChildren) {
        this.x = x
        this.y = y
        this.z = height - 1
        this.height = height
    ...
    setSpritePosition(numChildren) {
        let s = gameSession
        let xPos = s.tileset.tileFaceX * s.scale * this.x + s.offsetX
        let yPos = s.tileset.tileFaceY * s.scale * this.y + s.offsetY
     ...
     */
    setTile(img) {
        this.tile = scene.add.sprite(0, 0, img).setInteractive()
