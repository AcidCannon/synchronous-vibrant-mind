...
class Layout {
    ...
    addJsonLayer(layer, height) {
        ...
        for (let i = 0; i < layer.length; i++) {
            ...
            for (let j = 0; j < layer[i].length; j++) {
                if (layer[i][j] === 1) {
                    row.push(new TileNode(j, i, height, this.numChildren))
                } else {
                    row.push(null)
                }
            }
     ...
     * <p>
     * If the findEmpty flag is set to true then we find TileNodes with uninitialized sprites.
    ...
    /**
     * Checks to see if the provided tile is selectable within the layout
     * <p>
     * The Tile must have at least one open side and no parent tiles.
     * @param  {TileNode} tile  - The TileNode we are checking.
     * @return {Boolean}        - Whether the tile was found to be selectable
     */
    checkSelectable(tile) {
        if (tile === null) {
            return false
        }
        ...
        if (tile.x === 0 || tile.x === this.layers[tile.z][0].length - 1) {
            return true
        }
        try {
            if (this.layers[tile.z][tile.y][tile.x - 1] === null ||
                this.layers[tile.z][tile.y][tile.x + 1] === null) {
                return true
            }
        } catch (err) {
            return true
    ...
    removeTile(tilenode) {
        ...
        for (var i = 0; i < tilenode.children.length; i++) {
            ...
            if (tilenode.children[i].parents.length === 0) {
                ...
                //console.log(tilenode.children[i])
                //console.log(this.checkSelectable(tilenode.children[i]))
                if (this.checkSelectable(tilenode.children[i])) {
        ...
        for (var i = 0; i < neighbours.length; i++) {
            if (neighbours[i].parents.length === 0) {
                neighbours[i].selectable = true
                neighbours[i].unhighlightTile()
            }
        }
    ...
    generateTiles(counts = null, possible = null) {
        ...
        //initialize selectable tiles
        //console.log(this.roots)
        for (var i = 0; i < this.roots.length; i++) {
            if (this.checkSelectable(this.roots[i])) {
                this.roots[i].selectable = true
            }
        }
        ...
        for (var i = 0; i < this.roots.length; i++) {
            if (this.roots[i].height === this.height) {
                ...
                if (this.roots.length > 3) {
                    ...
                    var newTile = new TileNode(lowTile.x, lowTile.y, lowTile.height + 1, this.numChildren)
                    ...
                    var newTile = new TileNode(randX, randY, 1, this.numChildren)
